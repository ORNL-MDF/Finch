...
class SolidificationData
{
  ...
  public:
    ...
    SolidificationData() {}
    // constructor
    SolidificationData( const Inputs& inputs, Grid<memory_space, Scalar>& grid )
        ...
        , enabled_( inputs.sampling.enabled )
    {
        ...
        using entity_type = typename Grid<memory_space, Scalar>::entity_type;
        auto layout =
    ...
    void updateEvents( Grid<memory_space, Scalar>& grid, const double time )
    {
        ...
        Cabana::Grid::grid_parallel_for(
            ...
            KOKKOS_CLASS_LAMBDA( const int i, const int j, const int k ) {
                ...
                {
                    Scalar m = ( temp - liquidus_ ) / ( temp - temp0 );
                    m = Math::fmin( Math::fmax( m, Scalar( 0 ) ), Scalar( 1 ) );
