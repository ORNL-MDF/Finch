...
struct boundBox
{
        ...
        midPoint((minP.x + maxP.x) / 2.0, (minP.y + maxP.y) / 2.0)
    {
        ...
        edges =
        {
            {Point(minPoint.x, minPoint.y), Point(minPoint.x, maxPoint.y)},
            {Point(maxPoint.x, minPoint.y), Point(maxPoint.x, maxPoint.y)},
            {Point(minPoint.x, maxPoint.y), Point(maxPoint.x, maxPoint.y)},
            {Point(minPoint.x, minPoint.y), Point(maxPoint.x, minPoint.y)}
        };
    }
...
struct Path
{
    ...
    ) const
    {
        ...
        for (size_t i = 0; i < lines.size(); ++i)
        {
            ...
            {
                file << "1\t" << first.x << "\t" << first.y << "\t0\t"
                      << 0 << "\t" << dwell_time << "\n";
            }

            // raster
            file << "0\t" << second.x << "\t" << second.y << "\t0\t"
                  << power << "\t" << speed << "\n";
        }
