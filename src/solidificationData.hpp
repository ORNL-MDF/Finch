...
public:
        ...
        db(db)
    {
        ...
        events = view_double2D(
            Kokkos::ViewAllocateWithoutInitializing( "events" ),
            capacity, nCmpts );
    ...
    void update()
    {
        ...
        if (new_count >= capacity)
        {
            ...
            Kokkos::resize( Kokkos::WithoutInitializing, events, capacity, nCmpts );
    ...
    void write()
    {
        ...
        auto events_host = Kokkos::create_mirror_view_and_copy(
            Kokkos::HostSpace(), events );
        ...
        std::string filename
        (
            folder_name + "/data_" + std::to_string( grid.comm_rank ) + ".csv"
        );
